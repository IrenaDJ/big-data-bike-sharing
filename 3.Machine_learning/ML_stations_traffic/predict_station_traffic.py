...
def main(stations_indexer_path, weather_indexer_path, model_data_folder, station_id, hour,
    ...
    #Loading data
    columns = ["hour", 'dayofyear', 'month', 'air_temp', 'wind_speed', 'visibility', 'weather_class']
    vals = [(hour, dayofyear, month, air_temp, wind_speed, visibility, weather_class)]
    data = spark.createDataFrame(vals, columns)

    model = PipelineModel.load(model_data_folder)
    
    stringIndexer = StringIndexer.load(stations_indexer_path)
    data = stringIndexer.fit(data).transform(data)
    stringIndexer_weather = StringIndexer.load(weather_indexer_path)
    data = stringIndexer_weather.fit(data).transform(data)

    features =\
        VectorAssembler(inputCols=["hour", 'dayofyear', 'month', 'air_temp', 'wind_speed', 'visibility', 'weather_index', 'station_index'], outputCol="features")
    ...
    print("Predicted count is: {}".format(predictions))
...
if __name__ == "__main__":
    ...
    main(args.stations_indexer_path, args.weather_indexer_path, args.model_data_folder, args.station_id, args.hour, \
